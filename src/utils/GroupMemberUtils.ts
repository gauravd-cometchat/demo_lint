...
export class GroupMemberUtils {
  ...
  static getViewMemberOptions(
    ...
    if (group.getOwner() == groupMember.getUid()) {
      ...
      const loggedInUserScope: string =
        group.getOwner() == loggedInUser
          ? CometChatUIKitConstants.groupMemberScope.owner
          : (group.getScope() ?? CometChatUIKitConstants.groupMemberScope.participant);
      const isKickAllowed =
        !additionalConfigurations?.hideKickMemberOption &&
        _allowedGroupMemberOptions[loggedInUserScope + groupMember.getScope()][
          CometChatUIKitConstants.GroupMemberOptions.kick
        ];
      const isBanAllowed =
        !additionalConfigurations?.hideBanMemberOption &&
        _allowedGroupMemberOptions[loggedInUserScope + groupMember.getScope()][
          CometChatUIKitConstants.GroupMemberOptions.kick
        ];
      const ischangeScopeAllowed =
        !additionalConfigurations?.hideScopeChangeOption &&
        _allowedGroupMemberOptions[loggedInUserScope + groupMember.getScope()][
          CometChatUIKitConstants.GroupMemberOptions.kick
        ];
      if (isKickAllowed) {
  ...
  static getBannedMemberOptions(group: CometChat.Group, groupMember: CometChat.GroupMember) {
    if (
      _allowedGroupMemberOptions[group.getScope() + groupMember.getScope()][
        CometChatUIKitConstants.GroupMemberOptions.unban
      ]
    ) {
      return [
        new CometChatOption({
