    ...
    useEffect(() => {
      ...
      const ccMessageSent = CometChatMessageEvents.ccMessageSent.subscribe((data: IMessages) => {
        if (
          ...
          data.message.getReceiverId() == user.getUid() &&
...
const CometChatAIAssistantChatComponent = (props: AIAssistantChatProps) => {
  ...
  const displaySuggestions =
    suggestedMessages.length > 0
      ? suggestedMessages
      : ((user.getMetadata() as any)?.suggestedMessages as string[]);
      ...
      <div className="cometchat-ai-assistant-chat__header-auxiliary-view">
        ...
        {!hideChatHistory && (
          <div className="cometchat-ai-assistant-chat__header-auxiliary-view-chat-history">
            <CometChatButton
              hoverText={getLocalizedString('ai_assistant_chat_history_title')}
              onClick={onHistoryButtonClick}
              iconURL={chatHistoryIcon}
            />
          </div>
        )}
        {showCloseButton && (
          <div className="cometchat-ai-assistant-chat__header-auxiliary-view-close-button">
            <CometChatButton
              hoverText={getLocalizedString('thread_close_hover')}
              onClick={onCloseButtonClicked}
              iconURL={closeButtonIcon}
            />
          </div>
        )}
      </div>
      ...
      <div className="cometchat-ai-assistant-chat__empty-state-content">
        ...
        {emptyChatIntroMessageView || (
          <div className="cometchat-ai-assistant-chat__empty-state-intro-message">
            {(user.getMetadata() as any)?.introductoryMessage ??
              getLocalizedString('ai_assistant_chat_intro_message')}
          </div>
        )}
        {!hideSuggestedMessages && displaySuggestions && displaySuggestions.length > 0 && (
  ...
  return (
    <div className="cometchat" style={{ height: '100%', width: '100%', overflow: 'hidden' }}>
      <div className="cometchat-ai-assistant-chat__wrapper">
        ...
        <div
          className={`cometchat-ai-assistant-chat__sidebar ${isSidebarOpen ? 'cometchat-ai-assistant-chat__sidebar--open' : ''}`}
        >
