...
export class ConversationsManager {
  ...
  constructor(args: Args) {
    const { conversationsRequestBuilder, errorHandler } = args;
    ConversationsManager.errorHandler = errorHandler;

    const convRequestBuilder =
      conversationsRequestBuilder ||
      new CometChat.ConversationsRequestBuilder().setLimit(ConversationsManager.limit);
  ...
  static attachGroupListener(
    callback: (message: CometChat.BaseMessage, remove?: boolean) => Promise<void>,
   ...
   */
  static attachMessageReceivedListener(
    callback: (message: CometChat.BaseMessage) => Promise<void>
  ) {
    try {
   ...
   */
  static attachMessageReceiptListener(
    callback: (receipt: CometChat.MessageReceipt, updateReadAt: boolean) => void
  ) {
    try {
      const onMessagesDelivered = CometChatMessageEvents.onMessagesDelivered.subscribe(
      ...
      const onMessagesDeliveredToAll = CometChatMessageEvents.onMessagesDeliveredToAll.subscribe(
        (messageReceipt: CometChat.MessageReceipt) => {
          callback(messageReceipt, false);
        }
      );
      const onMessagesReadByAll = CometChatMessageEvents.onMessagesReadByAll.subscribe(
        (messageReceipt: CometChat.MessageReceipt) => {
          callback(messageReceipt, true);
        }
      );
   ...
   */
  static attachMessageTypingListener(
    callback: (typingIndicator: CometChat.TypingIndicator, typingStarted: boolean) => void
  ) {
    try {
  ...
  static shouldLastMessageAndUnreadCountBeUpdated = (message: CometChat.BaseMessage) => {
    try {
      ...
      if (
        ...
        !CometChatUIKit.conversationUpdateSettings?.shouldUpdateOnMessageReplies()
      ) {
        return false;
      }
      if (isCustomMessage) {
