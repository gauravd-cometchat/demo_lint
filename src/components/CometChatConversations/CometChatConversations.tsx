...
export function CometChatConversations(props: ConversationsProps) {
    ...
    async (fetchId: string, isConnected: boolean = false): Promise<void> => {
      try {
        ...
        if (conversations.length !== 0 && fetchNextIdRef.current === fetchId) {
          const removeOldConversation = isConnected ? true : false;
          dispatch({
            type: 'appendConversations',
            conversations,
  ...
  const hideConversation = useCallback(
    (
      conversationsRequest: ConversationsRequest | null,
      message: CometChat.BaseMessage
    ): boolean => {
      try {
        if (
          ...
          message.getReceiverType() !== conversationsRequest.getConversationType()
        )
          return true;
        ...
        if (messageReceiverType === CometChatUIKitConstants.MessageReceiverType.group) {
          ...
          if (
            messageSender.getRole() !== '@agentic' &&
            messageReceiver.getRole() !== '@agentic' &&
            isOnlyAgentic
          ) {
            return true;
          } else if (
            (messageSender.getRole() === '@agentic' || messageReceiver.getRole() === '@agentic') &&
            hideAgentic
          ) {
            return true;
          }
    ...
    async (message: CometChat.BaseMessage): Promise<void> => {
      try {
        ...
        if (
          ...
          !hideConversation(conversationsRequestBuilder?.build() || null, message) &&
          !(
            (message.getCategory() === CometChatUIKitConstants.MessageCategory.custom &&
              !getIncrementUnreadCountBoolFromMetaData(message)) ||
            (activeConversation &&
              activeConversation.getConversationId() === message.getConversationId())
          )
        ) {
  ...
  const setTypingIndicator = useCallback(
    (typingIndicator: CometChat.TypingIndicator, typingStarted: boolean): void => {
      try {
          ...
          <div className="cometchat-conversations__subtitle-text-wrapper">
            {isGroupSubtitle &&
              ...
              (lastMessage.getCategory() != CometChatUIKitConstants.MessageCategory.custom ||
                (lastMessage.getCategory() == CometChatUIKitConstants.MessageCategory.custom &&
                  lastMessage.getType() !== CometChatUIKitConstants.calls.meeting)) && (
                <span className={`cometchat-conversations__subtitle-text-sender`}>
                  {getLastMessageSenderName}:
                </span>
              )}
            <div className={`cometchat-conversations__subtitle-icon ${iconClass}`} />
            <div
  ...
  function getIconNameByCallType(message: CometChat.Call): string {
    try {
      let iconName = '';
      const isMissedCallMessage = isMissedCall(message as CometChat.Call, state.loggedInUser!);
    ...
    try {
      ...
      if (!options) {
        ...
        for (let i = 0; i < defaultOptions.length; i++) {
          if (
            defaultOptions[i].id === CometChatUIKitConstants.ConversationOptions.delete &&
  ...
  const getEmptyView = () => {
    ...
    return (
      <div className="cometchat-conversations__empty-state-view">
        <div className="cometchat-conversations__empty-state-view-icon">
  ...
  return (
    <div className="cometchat" style={{ width: '100%', height: '100%' }}>
      ...
      >
        ...
        {showToast ? (
          <CometChatToast text={getLocalizedString('conversation_deleted')} onClose={closeToast} />
        ) : null}
      </div>
    </div>
  );
