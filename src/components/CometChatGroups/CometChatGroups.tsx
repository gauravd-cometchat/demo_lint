...
export type Action =
  | { type: 'appendGroups'; groups: CometChat.Group[]; removeOldGroups?: boolean }
  ...
  switch (type) {
    case 'appendGroups':
      if (action.groups.length > 0) {
        ...
        const uniqueGroups = Array.from(
          new Map(groups.map((group) => [group.getGuid(), group])).values()
        );
        newState = { ...state, groupList: uniqueGroups };
      } else {
        newState = {
          ...state,
          groupList: Array.from(
            new Map(
              [...state.groupList, ...action.groups].map((group) => [group.getGuid(), group])
            ).values()
          ),
        };
      }
    ...
    case 'updateGroupForSDKEvents': {
      ...
      if (targetIdx > -1) {
        ...
        newState = {
          ...state,
          groupList: groupList.map((g, i) => (i === targetIdx ? updatedGroup : g)),
        };
      } else if (addGroup) {
        if (hasJoined !== undefined) {
...
export function CometChatGroups(props: GroupsProps) {
  ...
  (() => {
    if (
      state.searchText !== groupsSearchText.current &&
      state.searchText.trim().length > 0 &&
      state.searchText.trim().length == state.searchText.length
    ) {
   ...
   */
  const fetchNextAndAppendGroups = useCallback(
    async (fetchId: string): Promise<void> => {
      const groupsManager = groupsManagerRef.current;
      if (!groupsManager) {
        return;
      }
      const initialState = isConnectionReestablished.current ? States.loaded : States.loading;
      ...
      try {
        ...
        if (groups.length !== 0) {
          const removeOldGroups = isConnectionReestablished.current ? true : false;
        ...
        if (attachListenerOnFetch.current) {
          GroupsManager.attachConnestionListener(() => {
            const requestBuilder =
              groupsRequestBuilder === null
                ? new CometChat.GroupsRequestBuilder().setLimit(30)
                : groupsRequestBuilder;
            groupsManagerRef.current = new GroupsManager({
              searchText: groupsSearchText.current,
  ...
  function getSubtitleView(group: CometChat.Group): JSX.Element {
    try {
      ...
      return (
        <div className="cometchat-groups__subtitle">
    ...
    try {
      ...
      switch (selectionMode) {
        ...
        case SelectionMode.multiple: {
          return <CometChatCheckbox onCheckBoxValueChanged={(e) => onSelect?.(group, e.checked)} />;
        }
        default: {
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  function getListItem(): (group: CometChat.Group) => JSX.Element {
    ...
    return function (group: CometChat.Group) {
      try {
        ...
        return (
          <div
            className={`cometchat-groups__list-item ${hideGroupType ? '' : `cometchat-groups__list-item-${groupType}`}
  ...
  const getLoadingView = () => {
    try {
      ...
      return (
        <div className="cometchat-groups__shimmer">
      ...
      return (
        <div className="cometchat-groups__empty-state-view">
          ...
          <div className="cometchat-groups__empty-state-view-body">
            <div className="cometchat-groups__empty-state-view-body-title">
              {getLocalizedString('group_empty_title')}
            </div>
            <div className="cometchat-groups__empty-state-view-body-description">
              {getLocalizedString('group_empty_subtitle')}
            </div>
          </div>
        </div>
      );
      ...
      return (
        <div className="cometchat-groups__error-state-view">
          ...
          <div className="cometchat-groups__error-state-view-body">
            <div className="cometchat-groups__error-state-view-body-title">
              {getLocalizedString('group_error_title')}
            </div>
            <div className="cometchat-groups__error-state-view-body-description">
      ...
      >
        <CometChatList
          ...
          searchPlaceholderText={
            searchPlaceholderTextRef.current || getLocalizedString('group_search_placeholder')
          }
          ...
          state={
            state.fetchState === States.loaded && state.groupList.length === 0
              ? States.empty
              : state.fetchState
          }
