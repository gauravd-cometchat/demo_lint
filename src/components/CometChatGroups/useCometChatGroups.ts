...
export function useCometChatGroups(args: Args) {
  ...
  useEffect(
    ...
    () => {
      try {
        const groupCreatedSub = CometChatGroupEvents.ccGroupCreated.subscribe(
          (group: CometChat.Group) => {
            dispatch({ type: 'prependGroup', group: CometChatUIKitUtility.clone(group) });
          }
        );
        const groupDeletedSub = CometChatGroupEvents.ccGroupDeleted.subscribe(
          (group: CometChat.Group) => {
        ...
        const groupOwnershipChangedSub = CometChatGroupEvents.ccOwnershipChanged.subscribe(
          (item) => {
