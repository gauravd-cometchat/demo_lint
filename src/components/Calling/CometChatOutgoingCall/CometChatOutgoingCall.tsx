...
const CometChatOutgoingCall = (props: OutgoingCallProps) => {
  ...
  const subtitleText: string = getLocalizedString('calls_outgoing');
  /**
   ...
   */
  const getAvatarURL = () => {
    return callRef.current?.getReceiverType() === CometChatUIKitConstants.MessageReceiverType.user
      ? (callRef.current?.getReceiver() as CometChat.User)?.getAvatar()
      : (callRef.current?.getReceiver() as CometChat.Group)?.getIcon();
  };
  /**
      ...
      if (!disableSoundForCalls) {
        if (customSoundForCalls) {
          CometChatSoundManager.play(
            CometChatSoundManager.Sound.outgoingCall!,
            customSoundForCalls
          );
        } else {
  ...
  useCometChatOutgoingCall(errorHandler, playAudio, call);
              ...
              <div className="cometchat-outgoing-call">
                <div className="cometchat-outgoing-call__title-container">
                  ...
                  {subtitleView ? (
                    ...
                    <div className="cometchat-outgoing-call__subtitle">{subtitleText}</div>
                  )}
                </div>
                {avatarView ? (
                  avatarView
                  ...
                  <div className="cometchat-outgoing-call__avatar">
                    <CometChatAvatar
                      name={callRef.current?.getReceiver()?.getName()}
                      image={getAvatarURL()}
                    />
                  </div>
                )}
