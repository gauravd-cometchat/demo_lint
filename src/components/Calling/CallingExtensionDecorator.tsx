...
export class CallingExtensionDecorator extends DataSourceDecorator {
  ...
  override getAllMessageTemplates(additionalConfigurations?: any): CometChatMessageTemplate[] {
    ...
    if (
      !this.checkIfTemplateCategoryExist(templates, CometChatUIKitConstants.MessageCategory.call)
    ) {
  ...
  getDirectCallTemplate(): CometChatMessageTemplate {
    return new CometChatMessageTemplate({
      ...
      contentView: (message: CometChat.BaseMessage, _alignment: MessageBubbleAlignment) => {
        ...
        return this.getDirectCallMessageBubble(message as CometChat.CustomMessage, _alignment);
      },
      options: (
  ...
  getDefaultCallTemplate(): CometChatMessageTemplate[] {
    const templates: CometChatMessageTemplate[] = [
      new CometChatMessageTemplate({
        ...
        contentView: (message: CometChat.BaseMessage, _alignment: MessageBubbleAlignment) => {
          return this.getDefaultAudioCallMessageBubble(message as CometChat.Call, _alignment);
        },
        bottomView: (message: CometChat.BaseMessage, alignment: MessageBubbleAlignment) => {
          return super.getBottomView(message, alignment);
        },
      ...
      new CometChatMessageTemplate({
        ...
        category: CometChatUIKitConstants.MessageCategory.call,
        contentView: (message: CometChat.BaseMessage, _alignment: MessageBubbleAlignment) => {
          return this.getDefaultVideoCallMessageBubble(message as CometChat.Call, _alignment);
        },
        bottomView: (message: CometChat.BaseMessage, alignment: MessageBubbleAlignment) => {
          return super.getBottomView(message, alignment);
        },
  ...
  getDateFormat(): CalendarObject {
    ...
    const componentCalendarFormat = sanitizeCalendarObject(
      this.configuration?.callInitiatedDateTimeFormat
    );
  ...
  getDirectCallMessageBubble(
    ...
    const joinCallButtonText = getLocalizedString('message_list_join_call');
    const isMyMessage =
      !_message.getSender() || this.loggedInUser?.getUid() == _message.getSender().getUid();
    const isAudioCall =
      (_message.getCustomData() as any).callType == CometChatUIKitConstants.MessageTypes.audio;
    const audioIcon = isMyMessage ? outgoingAudioCall : incomingAudioCall;
    const videoIcon = isMyMessage ? outgoingVideoCall : incomingVideoCall;
    const callIcon = isAudioCall ? audioIcon : videoIcon;
    const callBubbleTitle = isAudioCall
      ? getLocalizedString('message_list_voice_call')
      : getLocalizedString('message_list_video_call');
    return (
      <CometChatCallBubble
        subtitle={
          <CometChatDate timestamp={_message.getSentAt()} calendarObject={this.getDateFormat()} />
        }
  ...
  startDirectCall(sessionId: string, message: CometChat.CustomMessage) {
    let callBuilder;
    const audioOnlyCall =
      (message.getCustomData() as any)?.callType == CometChatUIKitConstants.MessageTypes.audio;
    ...
    const ongoingCallScreen = (
      <CometChatOngoingCall
        callSettingsBuilder={callBuilder}
        sessionID={sessionId}
  ...
  getDefaultAudioCallMessageBubble(_message: CometChat.Call, _alignment: MessageBubbleAlignment) {
    return (
      <div className={this.getCallStatusClass(_message)}>
        <CometChatActionBubble messageText={this.getCallActionMessage(_message)} />
      </div>
    );
  }

  getDefaultVideoCallMessageBubble(_message: CometChat.Call, _alignment: MessageBubbleAlignment) {
    return (
      <div className={this.getCallStatusClass(_message)}>
        <CometChatActionBubble messageText={this.getCallActionMessage(_message)} />
      </div>
  ...
  override getLastConversationMessage(
    ...
    if (
      ...
      conversation.getLastMessage().category == CometChatUIKitConstants.MessageCategory.call
    ) {
      ...
      conversation?.getLastMessage() &&
      conversation.getLastMessage().type == CometChatUIKitConstants.calls.meeting
    ) {
      ...
      if (!message.getSender() || message?.getSender()?.getUid() == loggedInUser.getUid()) {
        if (callType === CometChatUIKitConstants.MessageTypes.audio) {
          actionMessage = getLocalizedString(
  ...
  override getAuxiliaryHeaderMenu(
    ...
    if (
      additionalConfigurations?.hideVideoCallButton &&
      additionalConfigurations?.hideVoiceCallButton
    ) {
      return [];
    }
    const callButtons = (
      <CometChatCallButtons
        ...
        outgoingCallConfiguration={
          this.configuration?.callButtonConfiguration?.outgoingCallConfiguration
        }
