...
const CometChatIncomingCall = (props: IncomingCallProps) => {
  ...
  const incomingcallListenerId: string = 'incomingcall_' + new Date().getTime(),
  ...
  const isCallActive = useCallback((call: CometChat.Call) => {
    ...
    try {
      if (StorageUtils.getItem(CometChatUIKitConstants.calls.activecall)) {
        const oldCall: any = StorageUtils.getItem(CometChatUIKitConstants.calls.activecall);
    ...
    (call: CometChat.Call) => {
      try {
        if (
          ...
          if (!disableSoundForCalls && !showOngoingCallScreen && !showOutGoingCallScreen) {
            setTimeout(() => {
              playAudio();
            }, 100);
          }
  ...
  const clearStoredActiveCall = () => {
    try {
      const currentActiveCall: CometChat.Call = StorageUtils.getItem(
      ...
      const ccCallRejected = CometChatCallEvents.ccCallRejected.subscribe(
        (call: CometChat.Call) => {
          ...
          if (
            ...
            call?.getSessionId() === currentOutgoingCallRef.current?.getSessionId()
          ) {
            setShowOngoingCallScreen(false);
            setShowOutGoingCallScreen(false);
  ...
  const getIsActiveCall = () => {
    try {
      return (
        ...
        StorageUtils.getItem(CometChatUIKitConstants.calls.activecall)
      );
    } catch (e) {
      ...
      CometChat.addCallListener(
        ...
        new CometChat.CallListener({
          onIncomingCallReceived: (call: CometChat.Call) => {
            if (getIsActiveCall()) {
              CometChat.rejectCall(call.getSessionId(), CometChatUIKitConstants.calls.busy).then(
                (rejectedCall: CometChat.Call) => {
                  CometChatCallEvents.ccCallRejected.next(rejectedCall);
                },
                (error: CometChat.CometChatException) => {
  ...
  function getCallBuilder(): typeof CometChatUIKitCalls.CallSettings {
    try {
      const audioOnlyCall: boolean =
        callRef.current?.getType() === CometChatUIKitConstants.MessageTypes.audio ? true : false;
  ...
  const ListItemSubtitleView = () => {
    return (
      <>
        {subtitleView ? (
          <div className="cometchat-incoming-call__subtitle">{subtitleView(callRef.current!)}</div>
        ) : (
          <div className="cometchat-incoming-call__subtitle">
            ...
            />
            <div className="cometchat-incoming-call__subtitle-text">{subtitleText}</div>
          </div>
        )}
      </>
    );
