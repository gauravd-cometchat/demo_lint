  ...
  switch (type) {
    case 'setConversationList': {
      ...
      for (let i = 0; i < conversationList.length; i++) {
        ...
        const convWithId =
          convWith instanceof CometChat.User ? convWith?.getUid() : convWith.getGuid();
    ...
    case 'addTypingIndicator': {
      ...
      const isReceiverTypeGroup =
        typingIndicator.getReceiverType() === CometChatUIKitConstants.MessageReceiverType.group;
    ...
    case 'updateConversationLastMessageAndGroupAndPlaceAtTheTop': {
      ...
      if (targetIdx > -1) {
        ...
        newState = {
          ...state,
          conversationList: [newConv, ...conversationList.filter((conv, i) => i !== targetIdx)],
        };
      }
      break;
    }
    ...
    case 'resetUnreadCountAndSetReadAtIfLastMessage': {
      ...
      if (targetIdx > -1) {
        const msg = conversationList[targetIdx].getLastMessage();
 ...
 */
export function useCometChatSearchConversationsList(
  props: UseCometChatSearchConversationsListProps
) {
  const {
    searchKeyword = '',
  ...
  const updateConversationList = useCallback(
    (conversation: CometChat.Conversation, newMessage: CometChat.BaseMessage): void => {
      try {
        ...
        if (
          ...
          message.getReceiverType() !== conversationsRequestBuilder.build().getConversationType()
        ) {
          return;
        }
  ...
  const onMessageReceived = useCallback(
    async (message: CometChat.BaseMessage): Promise<void> => {
      try {
        ...
        if (message.getSender().getUid() !== loggedInUser?.getUid() && !message.getDeliveredAt()) {
  ...
  useEffect(
    ...
    () => {
      if (!hideUserStatus) {
        return SearchConversationsManager.attachUserListener((user: CometChat.User) =>
  ...
  useEffect(
     ...
     */
    () => {
      return SearchConversationsManager.attachGroupListener(
        refreshSingleConversation,
        loggedInUser!
      );
    },
  ...
  useEffect(
     ...
     */
    () => {
      return SearchConversationsManager.attachMessageModifiedListener(
        (message: CometChat.BaseMessage) => {
  ...
  useEffect(
    ...
    () => {
      try {
        ...
        let builtBuilder: CometChat.ConversationsRequest;
        if (builder) {
          builtBuilder = builder.build();
        }
        const groupMemberScopeChangedSub = CometChatGroupEvents.ccGroupMemberScopeChanged.subscribe(
          (item) => {
            dispatch({
              type: 'updateConversationLastMessageAndPlaceAtTheTop',
        ...
        const messageSentSub = CometChatMessageEvents.ccMessageSent.subscribe((item) => {
          if (item.status === MessageStatus.success) {
            if (
              builtBuilder &&
              builtBuilder?.getConversationType() &&
              item.message.getReceiverType() !== builtBuilder?.getConversationType()
            ) {
              return;
            }
            CometChat.CometChatHelper.getConversationFromMessage(item.message).then(
              (conversation) => {
        ...
        const messageDeletedSub = CometChatMessageEvents.ccMessageDeleted.subscribe((message) => {
          dispatch({
            ...
            message: CometChatUIKitUtility.clone(message),
          }); // Cloning message since I don't know if the developer is passing a cloned copy
        });
  ...
  function getSubtitleReadReceiptView(conversation: CometChat.Conversation): JSX.Element | null {
    try {
      const lastMessageCategory = conversation.getLastMessage()
  ...
  function getIconNameByCallType(message: CometChat.Call): string {
    try {
      let iconName = '';
      const isMissedCallMessage = isMissedCall(message as CometChat.Call, loggedInUser!);
    ...
    try {
      ...
      if (currentLoggedInUser) {
        ...
        const isGroupSubtitle =
          lastMessage && conversation?.getConversationType() != CometChat.RECEIVER_TYPE.USER;
        const isMessageFromLoggedInUser =
          lastMessage?.getSender().getUid() == currentLoggedInUser.getUid();
        ...
        return (
          <div className="cometchat-search__conversations-subtitle-text-wrapper">
            ...
            <div
              className={`cometchat-search__conversations-subtitle-icon ${
                iconName
  ...
  const getListItem = useCallback(() => {
    ...
    return function (conversation: CometChat.Conversation) {
      try {
        const isActive =
          conversation.getConversationId() === activeConversationState?.getConversationId();
  ...
  const shouldRender = useCallback(() => {
    ...
    activeFilters,
    searchKeyword,
    conversationState.fetchState,
    conversationState.conversationList.length,
  ]);
  ...
  const renderConversationsList = () => {
    ...
    return (
      <div
        className={`cometchat-search__conversations ${useScrollPagination || activeFilters.length > 0 ? 'cometchat-search__conversations-full' : ''}`}
      >
        <CometChatList
