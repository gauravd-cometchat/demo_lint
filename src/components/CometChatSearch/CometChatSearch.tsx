...
export function CometChatSearch(props: SearchProps) {
  ...
  const getLoadingView = useCallback(() => {
    ...
    return (
      <div className="cometchat-search__shimmer">
        {[...Array((activeFilters && activeFilters.length > 0) || !!guid || !!uid ? 7 : 3)].map(
          (_, index) => (
  ...
  const shouldRenderConversations = useCallback(() => {
    ...
    if (!uid && !guid) {
      ...
      if (activeFilters.length > 0) {
        return (
          activeFilters.includes(CometChatSearchFilter.Conversations) ||
          activeFilters.includes(CometChatSearchFilter.Groups) ||
          activeFilters.includes(CometChatSearchFilter.Unread)
        );
      }
      return false;
    } else {
  ...
  const getAvailableFilters = useCallback(() => {
    ...
    const availableFilters: CometChatSearchFilter[] = [];
  ...
  const getVisibleFilters = useCallback(() => {
    ...
    if (uid || guid) {
      filteredAvailableFilters = availableFilters.filter(
        (filter) => !conversationFilters.includes(filter)
      );
    }
    ...
    const selectedContentFilters = activeFilters.filter((filter) =>
    ...
    if (hasMessagesFilter && selectedContentFilters.length > 0) {
      return [CometChatSearchFilter.Messages, ...selectedContentFilters].filter((filter) =>
        filteredAvailableFilters.includes(filter)
      );
    }
    ...
    if (effectiveSearchIn.includes(CometChatSearchScope.Conversations) && !uid && !guid) {
      const hasConversationFilter = activeFilters.some((filter) =>
        conversationFilters.includes(filter)
      );
      ...
      <div className="cometchat-search__header">
        {!hideBackButton && (
          <div className="cometchat-search__back-button">
            <CometChatButton aria-label="back" iconURL={BackIcon} onClick={onBack} />
          </div>
        )}
        ...
        <div className="cometchat-search__body-filters">
          {getVisibleFilters().map((id) => (
            <div
              ...
              key={id}
              className={`cometchat-search__body-filter cometchat-search__body-filter-${id}  ${
                activeFilters.includes(id) ? 'cometchat-search__body-filter-active' : ''
              }`}
            >
              <CometChatButton
                iconURL={activeFilters.includes(id) ? CloseIcon : filterIconsMap[id]}
                text={filterTextMap[id]}
              />
            </div>
          ))}
        </div>
