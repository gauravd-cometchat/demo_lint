  ...
  function processFile(file: File): Promise<File> {
    return new Promise((resolve, reject) => {
      try {
        ...
        reader.onerror = () => {
          reject(new Error(`Converting the file named "${file.name}" to binary failed`));
        };
...
*/
        ...
        if (parentMessageIdPropRef.current) {
          ...
          if (userPropRef.current) {
            if (
              ...
              receiverId === userPropRef.current.getUid()
            ) {
              return true;
            }
            ...
            if (
              ...
              receiverId === groupPropRef.current.getGuid()
            ) {
              return true;
            }
          }
  ...
  const [userMemberListType, setUserMemberListType] = useState<UserMemberListType | undefined>();
  ...
  const defaultMentionsItemClickHandler = (user: CometChat.User | CometChat.GroupMember | null) => {
    try {
      if (user) {
        const cometChatUsers = [user];
        mentionsTextFormatterInstanceRef.current.setCometChatUserGroupMembers(cometChatUsers);
        reRenderMentionsWithEntity(user);
      } else if (mentionAllLabel) {
        const cometChatChannels = [mentionAllLabel];
        mentionsTextFormatterInstanceRef.current.setCometChatMentionedChannels(cometChatChannels);
  ...
  useEffect(() => {
    try {
      const activePopoverSub = CometChatUIEvents.ccActivePopover.subscribe((id: string) => {
  ...
  useEffect(() => {
    const matchesMentionAll = !(
      mentionsSearchTerm &&
      mentionsSearchTerm.trim().length > 0 &&
      !mentionAllLabel.toLowerCase().startsWith(mentionsSearchTerm.trim().toLowerCase())
    );

    if (
      disableMentions &&
      (userMemberListType === UserMemberListType.users || !matchesMentionAll)
    ) {
      setShowListForMentions(false);
    }
  ...
  const searchMentions = useCallback(
    (searchTerm: any) => {
      try {
        ...
        const currentSearchTerm = searchTerm.split('@')[1].toLowerCase()
        ...
        if (
          (!currentSearchTerm ||
            !(
              ...
              currentSearchTerm.startsWith(lastEmptySearchTerm.current.toLowerCase())
            )) &&
  ...
  const reRenderMentionsWithEntity = useCallback(
    (entity: CometChat.User | CometChat.GroupMember | string) => {
      try {
        ...
        if (currentSelectionForRegex && textFormatterArray && textFormatterArray.length) {
          if (textFormatterArray && textFormatterArray.length) {
            for (let i = 0; i < textFormatterArray.length; i++) {
  ...
  const sendTextMessage = useCallback(
    async <T extends CometChat.TextMessage>(textMessage: T): Promise<T | undefined> => {
      try {
        for (let i = 0; i < textFormatterArray.length; i++) {
          textMessage = textFormatterArray[i].formatMessageForSending(textMessage) as T;
        }
        if (onTextChange) {
          onTextChange('');
        }
  ...
  const sendEditedTextMessage = useCallback(
    async <T extends CometChat.TextMessage>(editedTextMessage: T): Promise<T | undefined> => {
      try {
        for (let i = 0; i < textFormatterArray.length; i++) {
          editedTextMessage = textFormatterArray[i].formatMessageForSending(editedTextMessage) as T;
        }
  ...
  const handleEditTextMessageSend = useCallback(
    async (newText: string, textMessage: CometChat.TextMessage): Promise<void> => {
      try {
  ...
  const getMediaMessage = useCallback(
    async (file: File, fileType: MediaMessageFileType): Promise<CometChat.MediaMessage> => {
      try {
        const processedFile =
          fileType == CometChatUIKitConstants.MessageTypes.audio
  ...
  const sendMediaMessage = useCallback(
    async <T extends CometChat.MediaMessage>(mediaMessage: T): Promise<T | undefined> => {
      try {
  ...
  const handleSendVoiceMessage = useCallback(
    async (blob: Blob): Promise<void> => {
      ...
      voiceRecordingBtnRef.current?.closePopover();
      try {
        const audioFile = new File([blob], `${audioRecordingSimpleDateFormat()}.wav`, {
          type: blob.type,
        });
        handleMediaMessageSend(audioFile, CometChatUIKitConstants.MessageTypes.audio);
      } catch (error) {
  ...
  const handleMediaMessageSendWrapper = useCallback(async (): Promise<void> => {
    try {
      ...
      if (!mediaFilePickerElement?.files?.length || userPropRef.current?.getBlockedByMe()) {
        return;
      }
  ...
  function shouldShowSendButton(): boolean {
    ...
    return (
      !text ||
      (text && text.trim() === '') ||
      (state.textMessageToEdit !== null && state.textMessageToEdit.getText() === state.text)
    );
  }
  /**
    ...
    try {
      ...
      if (contenteditable?.textContent?.trim()) {
        let textToDispatch =
          contenteditable?.innerHTML?.trim() === '<br>'
            ? undefined
            : decodeHTML(contenteditable?.innerHTML.replace(/(<br>\s*)+$/, ''));
        if (contenteditable?.innerHTML?.trim() == '<br>') {
          contenteditable.innerHTML = '';
        }
        if (textFormatterArray && textFormatterArray.length) {
  ...
  function shouldShowAttachmentButton() {
    return (
      ...
      ChatConfigurator.getDataSource().getAttachmentOptions(getComposerId(), {
    ...
    try {
      const defaultSecondaryBtn = (
        <CometChatButton
          ...
          iconURL={state.contentToDisplay === 'attachments' ? PlusIconFill : PlusIcon}
        />
      );
      ...
      if (attachmentOptions && (user !== undefined || group !== undefined)) {
        actions = attachmentOptions;
      } else {
        actions = ChatConfigurator.getDataSource().getAttachmentOptions(getComposerId(), {
          hideAudioAttachmentOption,
          hideCollaborativeDocumentOption,
          hideCollaborativeWhiteboardOption,
          hideFileAttachmentOption,
    ...
    try {
      ...
      if (typeof actionOnClick === 'function') {
        ...
        if (shouldShowCustomMimeTypes()) {
          acceptMap[CometChatUIKitConstants.MessageTypes.image] =
            CometChatUIKitConstants.mimeTypes.image;
          acceptMap[CometChatUIKitConstants.MessageTypes.video] =
            CometChatUIKitConstants.mimeTypes.video;
          acceptMap[CometChatUIKitConstants.MessageTypes.audio] =
            CometChatUIKitConstants.mimeTypes.audio;
        }
  ...
  const createUniqueUUID = useMemo(() => {
    const parentMessageId = parentMessageIdPropRef.current
      ? parentMessageIdPropRef.current + '_'
      : '';
  ...
  function getHeaderView(): JSX.Element {
    ...
    const errorText = state.showMentionsCountWarning
      ? getLocalizedString('message_composer_mention_limit_warning')
      : getLocalizedString('message_composer_wrong_file_type');
    return (
      <div className="cometchat-message-composer__header">
        {state.showMentionsCountWarning || state.showValidationError ? (
          <div className="cometchat-message-composer__header-error-state">
            <div className="cometchat-message-composer__header-error-state-icon-wrapper">
              <div
                className="cometchat-message-composer__header-error-state-icon
         "
              ></div>
            </div>
            <span
              className="cometchat-message-composer__header-error-state-text
         "
            >
              {errorText}
            </span>
  ...
  function getVoiceRecordingView(): JSX.Element | null {
    const defaultSecondaryContent = (
      <CometChatMediaRecorder onSubmitRecording={handleSendVoiceMessage} autoRecording={true} />
    );
    ...
    const defaultSecondaryBtn = (
      <CometChatButton
        ...
        iconURL={state.contentToDisplay === 'voiceRecording' ? MicIconFill : MicIcon}
      />
    );

    return (
      <div
        className={`cometchat-message-composer__voice-recording-button ${state.contentToDisplay === 'voiceRecording' ? 'cometchat-message-composer__voice-recording-button-active' : ''}`}
      >
        <CometChatPopover
          useParentHeight={false}
          useParentContainer={true}
          ...
          content={state.contentToDisplay === 'voiceRecording' ? defaultSecondaryContent : null}
        >
          {defaultSecondaryBtn}
        </CometChatPopover>
      </div>
  ...
  function getAuxiliaryView(): JSX.Element | undefined {
    return auxiliaryButtonView ? (
      <div className="cometchat-message-composer__auxilary-button-view">{auxiliaryButtonView}</div>
    ) : undefined;
  }
  /**
  ...
  function getEmojiKeyboardView() {
    ...
    const defaultAuxiliaryBtn = (
      <CometChatButton
        ...
        iconURL={state.contentToDisplay === 'emojiKeyboard' ? SmileysIconFill : SmileysIcon}
      />
    );
    // Use default auxiliary content
    const defaultAuxiliaryContent = <CometChatEmojiKeyboard onEmojiClick={onEmojiClicked} />;
    return (
      <div
  ...
  function getTextMessageEditPreview(): JSX.Element | null {
    const checkForMentions = (message: CometChat.TextMessage) => {
      ...
      const cometChatUsersGroupMembers = [];
      const cometChatMentionedChannels = [];
  ...
  function getReplyMessagePreview(): JSX.Element | null {
    ...
    return (
      <CometChatMessagePreview
        ...
        previewTitle={ChatConfigurator.getDataSource().getMessagePreviewTitle(
          messageToReplyRef.current
        )}
        previewSubtitle={ChatConfigurator.getDataSource().getMessagePreviewSubtitle(
          messageToReplyRef.current
        )}
      />
    );
  ...
  const handleMouseDown = useCallback(
    (event: { target: any }) => {
      if (userMemberWrapperRef.current && !userMemberWrapperRef.current.contains(event.target)) {
        setShowListForMentions(false);
      }
    },
    [showListForMentions]
  ...
  const onKeyUp = useCallback(
    (event: any) => {
      ...
      if (keyUpCheck || (keyUpCheck && enterKeyBehavior == EnterKeyBehavior.None)) {
        event.preventDefault();
        return;
      }
  ...
  function isDescendant(sel: Selection): boolean {
    if (sel.rangeCount > 0) {
      ...
      const elm = getCurrentInput();
      const parentElm = range.commonAncestorContainer.parentNode;
      if (parentElm?.nodeName === 'SPAN') {
        return false;
      }

      if (elm?.contains(range.startContainer) && elm?.contains(range.endContainer)) {
        return true;
      } else {
        return false;
      }
        ...
        if (sel.current && range.current) {
          ...
          while ((node = el.firstChild)) {
            if (node instanceof HTMLElement) {
              if (textFormatterArray && textFormatterArray.length) {
                for (let i = 0; i < textFormatterArray.length; i++) {
                  node = textFormatterArray[i].registerEventListeners(node, node.classList);
                }
              }
    ...
    <>
      ...
      >
        {showListForMentions && (!disableMentionAll || !disableMentions) && (
          <div className="cometchat-mention-list" ref={userMemberWrapperRef}>
