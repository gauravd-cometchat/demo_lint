...
export class MessageListManager {
  ...
  constructor(
    ...
    if (messagesRequestBuilder) {
      ...
      const builder: CometChat.MessagesRequestBuilder = new CometChat.MessagesRequestBuilder()
        ...
        .setCategories(
          ChatConfigurator.dataSource.getAllMessageCategories({ hideGroupActionMessages })
        )
        .hideReplies(true)
        .setLimit(30);
  ...
  static attachListeners: (
    addStreamListener: boolean,
    callback: (key: string, mesage: CometChat.BaseMessage, group?: CometChat.Group) => void,
    addMessage: (message: CometChat.BaseMessage) => void,
    user?: CometChat.User
  ) => void = (
    addStreamListener: boolean = false,
    callback: (key: string, mesage: CometChat.BaseMessage, group?: CometChat.Group) => void,
    addMessage: (message: CometChat.BaseMessage) => void,
    user?: CometChat.User
  ) => {
          ...
          onAIAssistantEventReceived: (message: CometChat.AIAssistantBaseEvent) => {
            if (
              ...
              (!message.getConversationId().includes(user.getUid()) ||
                !message
                  .getConversationId()
                  .includes(CometChatUIKitLoginListener.getLoggedInUser()!.getUid()))
            ) {
              return;
            }
