...
export const CometChatMessageHeader = (props: MessageHeaderProps) => {
  ...
  function getDateFormat(): CalendarObject {
    ...
    const componentCalendarFormat = sanitizeCalendarObject(lastActiveAtDateTimeFormat);
  ...
  const updateSubtitle = useCallback(() => {
    try {
      ...
      if (user && !hideUserStatus) {
        ...
        const membersText = getLocalizedString(
          count > 1 ? 'message_header_members' : 'message_header_member'
        );
  ...
  const subscribeToEvents = useCallback(() => {
    try {
      ...
      const ccGroupLeft = CometChatGroupEvents.ccGroupLeft.subscribe((item: IGroupLeft) => {
    ...
    try {
      if (user.getStatus() === CometChatUIKitConstants.userStatusType.online) {
        ...
        if (user.getLastActiveAt()) {
          const date =
            user.getLastActiveAt().toString().length === 10
              ? user.getLastActiveAt()
              : Math.floor(user.getLastActiveAt() / 1000);
          const formattedDate = CometChatLocalize.formatDate(date, getDateFormat());
  ...
  const setTypingIndicatorText = useCallback(
      ...
      try {
        ...
        if (isTypingForCurrentChat(typing)) {
          ...
          if (loggedInUser?.getUid() == receiverId) {
            const isBlocked = new MessageUtils().getUserStatusVisible(userRef?.current);
            if (isBlocked) {
              return;
            }
  ...
  const isTypingForCurrentChat = (typing: CometChat.TypingIndicator) => {
    try {
      ...
      const receiverId = typing?.getReceiverId();
      return (
        (sender?.getUid() === userRef?.current?.getUid() &&
          loggedInUser?.getUid() === receiverId) ||
        (groupRef.current && groupRef.current.getGuid() == receiverId)
      );
    } catch (error) {
      ...
      CometChat.addGroupListener(
        ...
        new CometChat.GroupListener({
          onGroupMemberScopeChanged: (
            ...
            if (
              groupRef.current?.getGuid() === changedGroup?.getGuid() &&
              changedUser.getUid() === loggedInUser?.getUid()
            ) {
  ...
  const getSubtitleView = useCallback(() => {
    try {
      ...
      const userBlockedFlag = new MessageUtils().getUserStatusVisible(userRef.current);
      ...
      if (itemView) {
        ...
        return (
          <CometChatListItem
            ...
            avatarName={userRef.current?.getName() || groupRef.current?.getName()}
            avatarURL={userRef.current?.getAvatar() || groupRef.current?.getIcon() || ''}
            title={userRef.current?.getName() || groupRef.current?.getName() || ''}
    ...
    CometChatUIEvents.ccShowPanel.next({
      child: (
        <CometChatConversationSummary
          ...
          closeCallback={closeSummaryPanel}
        />
      ),
      position: PanelAlignment.messageListFooter,
      composerId: getComposerId(),
    });
  };
  /**
   * Shows the conversation summary button.
        ...
        <div className="cometchat-message-header__auxiliary-button-view">
          {
            ChatConfigurator.getDataSource().getAuxiliaryHeaderMenu(
              userRef.current,
              groupRef.current,
              { hideVideoCallButton, hideVoiceCallButton }
            ) as ReactNode
          }
  ...
  return (
    ...
    >
      <div className="cometchat-message-header">
        <div>
          {getBackButton()}
          <div className="cometchat-message-header__listitem">{getItemView()}</div>
        </div>
