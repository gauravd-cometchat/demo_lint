...
export const CometChatReactionList: React.FC<ReactionListProps> = ({
  ...
  const [requestBuilderMap, setRequestBuilderMap] = useState<
    Record<string, CometChat.ReactionsRequest>
  >({});
  ...
  useEffect(() => {
    selectedRecRef.current = selectedReaction;
  }, [selectedReaction]);
  ...
  useEffect(() => {
    ...
    onMessageReactionAdded = CometChatMessageEvents.onMessageReactionAdded.subscribe(
      (reactionReceipt) => {
        if (
          selectedRecRef.current == 'all' ||
          selectedRecRef.current == reactionReceipt.getReaction().getReaction()
        ) {
    ...
    (reaction: string) => {
      try {
        ...
        if (reactionsRequestBuilder) {
          ...
          requestBuilderRef.current = new CometChat.ReactionsRequestBuilder().setLimit(limit);
        }
  ...
  const showReactions = useCallback(async () => {
    try {
  ...
  const getReactionList = useCallback(
    async (requestBuilder: CometChat.ReactionsRequest, reaction: string) => {
      try {
        setState(States.loading);
  ...
  const showReactionsIfNeeded = useCallback(async () => {
    try {
  ...
  const fetchNext = useCallback(async () => {
    try {
      ...
      if (
        ...
        (reactionList[selectedReaction] && reactionList[selectedReaction].length === 0)
      ) {
        setIsFetching(false);
        return;
      } else {
   ...
   */
  const updateMessageToRemoveReactionLocally = (
    reaction: CometChat.Reaction,
    isMyReaction: boolean = true
  ) => {
    try {
      ...
      if (message) {
        ...
        const index = reactions.findIndex(
          (r) => r.getReaction() === reaction.getReaction() && r.getReactedByMe() == isMyReaction
        );
        if (index !== -1) {
  ...
  const updateCurrentUIList = (
    reactions: ReactionCount[],
    removedReaction?: CometChat.Reaction
  ) => {
    ...
    if (selectedReaction) {
      ...
      setReactionList((prev) => ({
        ...prev,
        [selectedReaction]: updatedActiveTabReactionsFiltered,
        ['all']: updatedReactions,
      }));
    ...
    try {
      ...
      return reactionsObject.map((reactionObject) => (
        <div
          ...
          onClick={() => {
            setSelectedReaction(reactionObject.id);
          }}
          key={reactionObject.id}
        >
          <div
            className={`cometchat-reaction-list__tabs-tab-emoji ${selectedReaction === reactionObject.id && 'cometchat-reaction-list__tabs-tab-emoji-active'}`}
          >
            {reactionObject.reaction}
          </div>
          <div
            className={`cometchat-reaction-list__tabs-tab-count ${selectedReaction === reactionObject.id && 'cometchat-reaction-list__tabs-tab-count-active'}`}
          >
  ...
  useEffect(() => {
    if (
      !isFirstRender &&
      !isFetching &&
      requestBuilderRef.current?.limit &&
      currentUIList.length < requestBuilderRef.current.limit
    ) {
      fetchNext();
    }
    ...
    <div className="cometchat cometchat-reaction-list" ref={parentRef}>
      <div>
        ...
        {state === States.loading ? (
          ...
          <div className="cometchat-reaction-list__error">{getLocalizedString('ERROR_TEXT')}</div>
        ) : state === States.loaded ? (
          <div
