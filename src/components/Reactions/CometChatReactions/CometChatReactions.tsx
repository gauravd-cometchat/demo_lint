...
export const CometChatReactions: React.FC<ReactionsProps> = ({
  ...
  const moreCount = useMemo(() => {
    try {
      ...
      return totalReactions > visibleCount ? totalReactions - visibleCount : 0;
    } catch (error) {
    ...
    return (
      ...
      >
        <div className="cometchat-reactions__more-reaction">
          <div className="cometchat-reactions__more-reaction-count">+{moreCount}</div>
        </div>
      </CometChatPopover>
    );
  }, [
    ...
    return (
      <>
        {visibleReactions.map((reaction, index) => (
          <div
            className="cometchat-reactions-info-wrapper"
            key={`${reaction.getReaction()}-${index}`}
          >
            <CometChatPopover
              useParentContainer={true}
              showOnHover={true}
  ...
  const reactionChildUi = (reaction: CometChat.ReactionCount) => {
    return (
      ...
      >
        <span className="cometchat-reactions__reaction-emoji">{reaction.getReaction()}</span>
        <span className="cometchat-reactions__reaction-count">{reaction.getCount()}</span>
      </button>
    );
  };
