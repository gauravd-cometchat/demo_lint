...
export const CometChatReactionInfo: React.FC<ReactionInfoProps> = ({
  ...
  const checkVisibilityOfElement = useCallback(() => {
    try {
      ...
      observerRef.current.observe(
        getCurrentDocument().getElementById('reaction-info-container') as Element
      );
    } catch (error) {
  ...
  const fetchReactionsOnInteraction = useCallback(async () => {
    try {
  ...
  const fetchReactions = async () => {
    try {
      ...
      const builder: ReactionsRequestBuilder = new CometChat.ReactionsRequestBuilder().setLimit(
        limit
      );
      const reactionBuilder = builder
        .setMessageId(messageObject?.getId())
  ...
  const getLoadedStateView = useCallback(() => {
    ...
    return (
      <div className="cometchat-reaction-info__emoji-text">
        <span className="cometchat-reaction-info__emoji">{reaction}</span>
        <div>
  ...
  const updatePlacementBasedOnPosition = () => {
    try {
      ...
      const viewportHeight = getCurrentWindow().innerHeight,
        viewportWidth = getCurrentWindow().innerWidth;
  ...
  return (
    <div
      ...
      style={
        placementState === Placement.top || placementState === Placement.bottom
