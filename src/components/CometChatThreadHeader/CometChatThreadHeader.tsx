...
const CometChatThreadHeader = (props: CometChatThreadHeaderProps) => {
  ...
  const addListener = useCallback(() => {
    try {
      ...
      const onMessageEdited = CometChatMessageEvents.onMessageEdited.subscribe(
        (message: CometChat.BaseMessage) => {
          setUpdatedMessage((prevMessage) => {
            if (
              message.getId() == prevMessage.getId() &&
              (!prevMessage.getEditedAt() || prevMessage.getEditedAt() != message.getEditedAt())
            ) {
              return message;
            }
            return prevMessage;
      ...
      const onFormMessageReceived = CometChatMessageEvents.onFormMessageReceived.subscribe(
      ...
      const onCardMessageReceived = CometChatMessageEvents.onCardMessageReceived.subscribe(
        (message: CometChat.InteractiveMessage) => {
          if (
            message?.getParentMessageId() &&
            message.getParentMessageId() == parentMessage.getId()
      ...
      const ccMessageDeleted = CometChatMessageEvents.ccMessageDeleted.subscribe(
        (message: CometChat.BaseMessage) => {
          setUpdatedMessage((prevMessage) => {
            if (
              message.getId() == prevMessage.getId() &&
              (!prevMessage.getDeletedAt() || prevMessage.getDeletedAt() != message.getDeletedAt())
            ) {
              return message;
            }
            return prevMessage;
      ...
      if (updatedMessage && loggedInUser.current) {
        if (messageBubbleView) return messageBubbleView;
          ...
          const bubbleTemplate =
            ...
            templatesArray?.find(
              (template: CometChatMessageTemplate) =>
                template.type === updatedMessage.getType() &&
                template.category === updatedMessage.getCategory()
            );
          if (!bubbleTemplate) {
            return <></>;
          }
  ...
  const getAdditionalClassName = useCallback(() => {
    try {
      const messageTypes = [
        ...
        StickersConstants.sticker,
      ];
      if (updatedMessage && messageTypes.includes(updatedMessage.getType()))
        return 'cometchat-thread-header__message-small';
    } catch (error) {
  ...
  return (
    <div className="cometchat">
      <div
        className={`cometchat-thread-header ${!showScrollbar ? 'cometchat-thread-header-hide-scrollbar' : ''}`}
      >
        <div className="cometchat-thread-header__top-bar">
          ...
          <div className="cometchat-thread-header__top-bar-close">{getCloseBtnView()}</div>
        </div>
        <div className="cometchat-thread-header__body">
          ...
          <div
            className={`cometchat-thread-header__message ${updatedMessage.getSender()?.getUid() !== loggedInUser.current?.getUid() ? 'cometchat-thread-header__message-incoming' : 'cometchat-thread-header__message-outgoing'} ${getAdditionalClassName()}`}
          >
            {getBubbleView()}
          </div>
            ...
            {!hideReplyCount && (
              <div className="cometchat-thread-header__reply-bar-count">
                ...
                {replyCount === 0 || replyCount > 1
                  ? getLocalizedString('thread_replies')
                  : getLocalizedString('thread_reply')}
              </div>
            )}
