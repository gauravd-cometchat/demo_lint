...
const CometChatButton = (props: ButtonProps) => {
  const { text, hoverText, iconURL, disabled, isLoading = false, onClick = () => {} } = props;
  ...
  return (
    <div className="cometchat">
      <button
        ...
        title={hoverText}
        onClick={(event: CustomEvent<{ event: PointerEvent }> & MouseEvent<HTMLButtonElement>) =>
          onClick(event)
        }
      ...
      >
        {isLoading ? (
          ...
          <>
            <div
              style={
                iconURL ? { WebkitMask: `url(${iconURL}) center center no-repeat` } : undefined
              }
