...
const CometChatConversationSummary = (props: CometChatConversationSummaryProps) => {
  const { getConversationSummary, closeCallback } = { ...defaultProps, ...props };
  ...
  function getErrorView(): JSX.Element | null {
    return <div className="cometchat-conversation-summary__error-view">{errorStateText}</div>;
  }
  ...
  function getEmptyView(): JSX.Element {
    return <div className="cometchat-conversation-summary__empty-view">{emptyStateText}</div>;
  }
  ...
  async function getLoadedView(conversationSummary: string): Promise<JSX.Element> {
    return new Promise((resolve, reject) => {
      try {
        const conversationSummaryView = <React.Fragment>{conversationSummary}</React.Fragment>;
      ...
      <div className="cometchat-conversation-summary__wrapper">
        <div className="cometchat-conversation-summary">
          <div className="cometchat-conversation-summary__header">
            <div className="cometchat-conversation-summary__header-title">{titleText}</div>
            <button
