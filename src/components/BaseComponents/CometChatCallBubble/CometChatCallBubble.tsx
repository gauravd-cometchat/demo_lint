...
const CometChatCallBubble = (props: CallBubbleProps) => {
  ...
  return (
    <div className="cometchat">
      <div
        className={`cometchat-call-bubble ${isSentByMe ? 'cometchat-call-bubble-outgoing' : 'cometchat-call-bubble-incoming'}`}
      >
        <div className="cometchat-call-bubble__body">
          <div
            className="cometchat-call-bubble__icon-wrapper
"
          >
            <div
              className="cometchat-call-bubble__icon-wrapper-icon"
              style={
                iconURL
                  ? {
          ...
          <div className="cometchat-call-bubble__body-content">
            <div
              className="cometchat-call-bubble__body-content-title
"
            >
              {title}
            </div>
        ...
        {buttonText?.trim() && buttonText.trim()?.length > 0 ? (
          <button
            className="cometchat-call-bubble__button"
            title={buttonText}
            onClick={() => (onClicked ? onClicked(sessionId) : null)}
          >
            {' '}
            {buttonText}
          </button>
