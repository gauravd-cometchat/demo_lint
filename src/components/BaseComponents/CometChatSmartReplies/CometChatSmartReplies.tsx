...
const CometChatSmartReplies = (props: CometChatSmartRepliesProps) => {
  const { getSmartReplies, onSuggestionClicked, closeCallback } = { ...defaultProps, ...props };
  ...
  function getErrorView(): JSX.Element {
    return <div className="cometchat-smart-replies__error-view">{errorStateText}</div>;
  }
        ...
        const loadedView = (
          <div className="cometchat-smart-replies__items-container">
            {replies.map((reply, index) => (
              <div
                key={index}
                className="cometchat-smart-replies__item"
                onClick={() => onSuggestionClicked && onSuggestionClicked(reply)}
              >
                <button className="cometchat-smart-replies__item-button">{reply}</button>
              </div>
            ))}
          </div>
        );
      ...
      <div className="cometchat-smart-replies__wrapper">
        <div className="cometchat-smart-replies">
          <div className="cometchat-smart-replies__header">
            <div className="cometchat-smart-replies__header-title">{titleText}</div>
            <button
