...
export const useCometChatTextBubble = (props: {
  textFormatters: Array<CometChatTextFormatter>;
}) => {
  const { textFormatters } = props;
        ...
        if (node.nodeType === Node.TEXT_NODE) {
          ...
          if (element.tagName.toLowerCase() === 'span') {
            ...
            if (hasClassName) {
              ...
              const processChildNodes = (parentElement: Element, targetElement: HTMLElement) => {
                    ...
                    if (childElement.tagName.toLowerCase() === 'span') {
                      ...
                      for (let i = 0; i < childElement.attributes.length; i++) {
                        ...
                        if (
                          ...
                          nestedSpan.setAttribute(attr.name, attr.value);
                        }
                      }
                      ...
                      targetElement.appendChild(document.createTextNode(childElement.outerHTML));
                    }
                  }
                });
              };
              ...
              if (textFormatters && textFormatters.length) {
                for (let i = 0; i < textFormatters.length; i++) {
                  textFormatters[i].registerEventListeners(spanElement, spanElement.classList);
                }
              }
