...
const CometChatContextMenu = (props: ContextMenuProps) => {
  ...
  const getMoreButton = useCallback(() => {
    return (
      ...
      >
        <div className="cometchat-menu-list__sub-menu-icon" />
      </div>
    );
  ...
  const getMenu = useCallback(
    (
      menu: Array<CometChatActionsIcon | CometChatActionsView | CometChatOption>,
      isSubMenu: boolean
    ) => {
      if (menu.length > 0) {
        return menu?.map((menuData, index: number) => {
          ...
          const className =
            (index == 0 || menu.length <= 2) && !isSubMenu ? '' : 'cometchat-menu-list__menu';
          let menuButton,
            moreButton = null;
                  ...
                  <div slot="children">
                    ...
                    >
                      <div
                        className={
                          isSubMenu
                            ? `cometchat-menu-list__sub-menu-list-item-icon cometchat-menu-list__sub-menu-list-item-icon-${menuData.id}`
                            : `cometchat-menu-list__main-menu-item-icon cometchat-menu-list__main-menu-item-icon-${menuData.id}`
                        }
                        style={
                          menuData?.iconURL
                            ? {
                                WebkitMask: `url(${menuData?.iconURL}) center center no-repeat`,
                                display: 'flex',
                              }
                            : undefined
                        }
                      ...
                      {isSubMenu ? (
                        <label
                          className={`cometchat-menu-list__sub-menu-item-title cometchat-menu-list__sub-menu-item-title-${menuData.id}`}
                        >
                          {menuData?.title}
                        </label>
                      ) : (
                ...
                <div
                  ...
                  onClick={() => {
                    ...
                    if (onOptionClicked) {
                      ...
                      menuData instanceof CometChatOption ||
                      menuData instanceof CometChatActionsIcon
                    ) {
                      if (menuData?.onClick) {
                        menuData.onClick();
                      }
        ...
        return <div className="cometchat-menu-list__menu-wrapper">{moreButton}</div>;
      }
    },
  ...
  const getAvailablePlacement = useCallback(
    (rect: DOMRect, height: number) => {
      ...
      return spaceBelow >= height + 10
        ? Placement.bottom
        : spaceAbove >= height + 10
          ? Placement.top
          : placement;
    },
  ...
  const getPopoverPositionStyle = useCallback(() => {
    if (useParentContainer) {
      parentViewRef.current =
        parentViewRef.current || (getTopMostCometChatElement() as HTMLDivElement | null);
  ...
  return (
    <div className="cometchat">
      <div className="cometchat-menu-list">
        <div className="cometchat-menu-list__main-menu">{getTopMenu()}</div>
