...
const CometChatDocumentBubble = (props: DocumentBubbleProps) => {
  ...
  return (
    <div className="cometchat">
      <div
        className={`cometchat-document-bubble ${isSentByMe ? 'cometchat-document-bubble-outgoing' : 'cometchat-document-bubble-incoming'}`}
      >
        <div className="cometchat-document-bubble__banner-image">
          <img src={bannerImage} alt="" />
        </div>
        <div className="cometchat-document-bubble__body">
          <div className="cometchat-document-bubble__body-icon"></div>
          <div className="cometchat-document-bubble__body-content">
            <div className="cometchat-document-bubble__body-content-name">
              <label title={title}>{title}</label>
            </div>
            {subtitle?.trim() && subtitle.trim()?.length > 0 ? (
              <div className="cometchat-document-bubble__body-content-description">
