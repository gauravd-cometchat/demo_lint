...
const CometChatMediaRecorder: React.FC<MediaRecorderProps> = ({
  ...
  const handleStartRecording = async () => {
    pauseActiveMedia();
    const hasAudioInput = await navigator.mediaDevices
      .enumerateDevices()
  ...
  const handlePauseRecording = () => {
    setIsPaused(true);
    pauseTimer();
    if (mediaRecorder) (mediaRecorder as MediaRecorder).pause();
  ...
  useEffect(() => {
    if (
      ...
      !isRecording &&
      autoRecording &&
      !userCancelledRecording.current
    ) {
  ...
  return (
    ...
    >
      {hasError ? (
        <div className="cometchat-media-recorder__error">
          <div className="cometchat-media-recorder__error-icon-wrapper">
            <div className="cometchat-media-recorder__error-icon"></div>
          </div>
          <div className="cometchat-media-recorder__error-content">
            <div className="cometchat-media-recorder__error-content-title">
              {getLocalizedString('media_recorder_error_title')}
            </div>
            <div className="cometchat-media-recorder__error-content-subtitle">
              {getLocalizedString('media_recorder_error_subtitle')}
            </div>
          </div>
        </div>
      ) : null}
      <div
        className="cometchat-media-recorder"
        style={{
          ...(hasError && {
            borderRadius: `0px 0px ${getThemeVariable('--cometchat-radius-4')} ${getThemeVariable('--cometchat-radius-4')}`,
          }),
        }}
      >
        {!mediaPreviewUrl ? (
          <div
            className="cometchat-media-recorder__recording"
            style={{
              ...(hasError && {
                borderRadius: `0px 0px ${getThemeVariable('--cometchat-radius-4')} ${getThemeVariable('--cometchat-radius-4')}`,
              }),
          ...
          >
            {isRecording ? (
              <div className="cometchat-media-recorder__recording-preview">
                ...
                <div className="cometchat-media-recorder__recording-preview-duration">
                  {formatTime(counter)}
                </div>
              </div>
            ) : isPaused ? (
