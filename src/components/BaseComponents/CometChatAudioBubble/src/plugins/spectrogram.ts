  ...
  switch (windowFunc) {
    case 'bartlett':
      for (i = 0; i < bufferSize; i++) {
        this.windowValues[i] =
          (2 / (bufferSize - 1)) * ((bufferSize - 1) / 2 - Math.abs(i - (bufferSize - 1) / 2));
      }
      break;
    case 'bartlettHann':
      for (i = 0; i < bufferSize; i++) {
...
class SpectrogramPlugin extends BasePlugin<SpectrogramPluginEvents, SpectrogramPluginOptions> {
  ...
  private createMelFilterBank(numMelFilters, sampleRate) {
    ...
    const melFilterBank = Array.from({ length: numMelFilters }, () =>
      Array(this.fftSamples / 2 + 1).fill(0)
    );
    const scale = sampleRate / this.fftSamples;
    for (let i = 0; i < numMelFilters; i++) {
      const hz = this.melToHz(melMin + (i / numMelFilters) * (melMax - melMin));
    ...
    for (let c = 0; c < channels; c++) {
      ...
      while (currentOffset + fftSamples < channelData.length) {
        ...
        for (let j = 0; j < fftSamples / 2; j++) {
          ...
          const valueDB = 20 * Math.log10(spectrum[j]);
