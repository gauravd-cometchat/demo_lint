...
export function useCometChatList(args: Args) {
  ...
  useEffect(
    ...
    () => {
      ...
      async function observerCallBack(entry: IntersectionObserverEntry[]) {
        ...
        if (
          !relevantEntry.isIntersecting ||
          stopCallingOnScrolledToBottomCallback ||
  ...
  useEffect(
    ...
    () => {
      ...
      async function observerCallBack(entry: IntersectionObserverEntry[]) {
        ...
        if (
          !relevantEntry.isIntersecting ||
          stopCallingOnScrolledToTopCallback ||
          rootElement.scrollHeight <= rootElement.clientHeight
        ) {
          return;
        }
  ...
  useEffect(
    ...
    () => {
      ...
      if (
        intersectionObserverRootRef.current.scrollHeight >
        intersectionObserverRootRef.current.clientHeight
      ) {
        if (
          Math.round(
            intersectionObserverRootRef.current.scrollHeight - scrollHeightTupleRef.current[1]
          ) !== 0
        ) {
          ...
          if (
            !scrollToBottom &&
            didTopObserverCallbackRunRef.current &&
            scrollHeightTupleRef.current[0] !== 0
          ) {
            intersectionObserverRootRef.current.scrollTop = Math.max(
    ...
    () => {
      setTimeout(() => {
        ...
        const isRootScrollable =
          (rootElement?.scrollHeight ?? 0) > (rootElement?.clientHeight ?? 0);
        const shouldRootScrollToBottom =
          scrollToBottom &&
          isRootScrollable &&
          intersectionObserverBottomTargetRef.current !== null;
