  ...
  (
    ...
    const getAvailablePlacement = useCallback(
      (rect: DOMRect, height: number) => {
        ...
        if (useParentContainer) {
          ...
          if (placement === Placement.top && spaceAboveParent >= height + (!showTooltip ? 10 : 0))
            return Placement.top;
          if (
            placement === Placement.bottom &&
            spaceBelowParent >= height + (!showTooltip ? 10 : 0)
          )
            return Placement.bottom;
          if (placement === Placement.left && spaceLeftParent >= height + 10) return Placement.left;
          if (placement === Placement.right && spaceRightParent >= height + 10)
    ...
    const calculatePopoverPosition = useCallback(() => {
      ...
      if ([Placement.top, Placement.bottom].includes(availablePlacement)) {
        positionStyle.top =
          availablePlacement === Placement.top
    ...
    const getPopoverPositionStyle = useCallback(() => {
      ...
      if ([Placement.top, Placement.bottom].includes(availablePlacement)) {
        ...
        positionStyle.left =
          rect.left + width - 10 > viewportWidth
        ...
        positionStyle.top =
          rect.top + height - 10 > viewportHeight
      ...
      <div className="cometchat">
        <div className="cometchat-popover">
          ...
          {isOpen && (
            ...
            >
              {showTooltip && availablePositionRef.current == Placement.top ? (
                <div className={`cometchat-popover__content-tooltip-top`}></div>
              ) : null}

              {content}
              {showTooltip && availablePositionRef.current == Placement.bottom ? (
                <div className={`cometchat-popover__content-tooltip-bottom`}></div>
              ) : null}
            </div>
          )}
        </div>
