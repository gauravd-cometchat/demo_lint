...
const CometChatListItem = (props: ListItemProps) => {
  ...
  const { listItemClick, isHovering, hideTail, showTail } = useCometChatListItem({
    onListItemClicked,
    id,
    menuRef,
  });
  ...
  return (
    ...
    >
      ...
      >
        {!leadingView ? (
          avatarName?.trim()?.length > 0 || avatarURL?.trim()?.length > 0 ? (
            <div className="cometchat-list-item__leading-view">
              <CometChatAvatar image={avatarURL} name={avatarName} />
        ...
        <div className="cometchat-list-item__body">
          <div className="cometchat-list-item__title-container">
            {titleView ?? <div className="cometchat-list-item__body-title">{title}</div>}
            <div className="cometchat-list-item__body-subtitle">{subtitleView}</div>
          </div>
          <div
            ...
            className={
              !isHovering || (isHovering && !menuView)
